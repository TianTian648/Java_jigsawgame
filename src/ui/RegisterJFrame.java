...
public class RegisterJFrame extends JFrame implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == reset) {
            ...
            verification(username.getText(), password.getText(), rePassword.getText());
        }
    }
    ...
    private void verification(String usr, String pwd1, String pwd2) {
        if (usr.length() == 0 || pwd1.length() == 0 || pwd2.length() == 0) {
    ...
    private boolean isRepeat(String usr) {
        for (User allUser : allUsers) {
            if (usr.equals(allUser.getUsername())) {
                return true;
            }
        }
        return false;
    ...
    private void insert(String usr, String pwd1) {
        //添加进去
        ArrayList<String> str = new ArrayList<>();
        for (User allUser : allUsers) {
            str.add(allUser.toString());
        }
        User user = new User(usr, pwd1);
        str.add(user.toString());
        FileUtil.writeLines(str, new File("userInfo.txt"), "UTF-8");
        showDialog("注册成功！");
        clearField();
